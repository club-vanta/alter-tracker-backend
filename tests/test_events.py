...
from app.models.models import EventLog, EventType, Organization, OrgRole
from tests.conftest import make_guest, make_meetup, make_org_member, make_rsvp, make_user
...
# ======================================================================
# GET /organizations/{org_id}/events/ - ADMIN ONLY
# ======================================================================


class TestListAllEvents:
    """Tests for GET /organizations/{org_id}/events/ (org admin or site admin only)."""
    ...
    def test_list_all_events_as_staff_returns_403_forbidden(
        self, client: TestClient, staff_headers: dict, org: Organization
    ):
        """Staff cannot access the org events list (requires org ADMIN or SITE_ADMIN)."""
        resp = client.get(f"/organizations/{org.id}/events/", headers=staff_headers)
        assert resp.status_code == status.HTTP_403_FORBIDDEN

    def test_list_all_events_without_token_returns_401_unauthorized(self, client: TestClient, org: Organization):
        """Unauthenticated requests are rejected."""
        resp = client.get(f"/organizations/{org.id}/events/")
    ...
    def test_list_all_events_invalid_type_returns_400(self, client: TestClient, admin_headers: dict, org: Organization):
        """Invalid event type returns 400."""
        resp = client.get(f"/organizations/{org.id}/events/?type=INVALID", headers=admin_headers)
    ...
    def test_list_all_events_pagination(
        ...
        # Get first page
        resp = client.get(f"/organizations/{org.id}/events/?limit=2&offset=0", headers=admin_headers)
        data = resp.json()
        assert data["total"] == 5
        assert data["limit"] == 2
        ...
        # Get second page
        resp = client.get(f"/organizations/{org.id}/events/?limit=2&offset=2", headers=admin_headers)
        data = resp.json()
        assert len(data["events"]) == 2


# ======================================================================
# GET /organizations/{org_id}/events/meetups/{id} - MEETUP EVENTS
# ======================================================================


class TestListMeetupEvents:
    """Tests for GET /organizations/{org_id}/events/meetups/{id}."""
    ...
    def test_list_meetup_events_as_staff_returns_200_ok(
        ...
        resp = client.get(f"/organizations/{org.id}/events/meetups/{meetup.id}", headers=staff_headers)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert data["total"] == 1
    ...
    def test_list_meetup_events_nonexistent_meetup_returns_404(
        ...
        make_org_member(session, org=org, user=staff_user, role=OrgRole.STAFF)
        fake_id = uuid.uuid4()
        resp = client.get(f"/organizations/{org.id}/events/meetups/{fake_id}", headers=staff_headers)
        assert resp.status_code == status.HTTP_404_NOT_FOUND
...
# ======================================================================
# GET /organizations/{org_id}/events/guests/{id} - GUEST EVENTS
# ======================================================================


class TestListGuestEvents:
    """Tests for GET /organizations/{org_id}/events/guests/{id}."""
...
# ======================================================================
# GET /organizations/{org_id}/events/staff/{id} - STAFF EVENTS
# ======================================================================


class TestListStaffEvents:
    """Tests for GET /organizations/{org_id}/events/staff/{id}."""
    ...
    def test_list_staff_events_own_events_returns_200_ok(
        ...
        resp = client.get(f"/organizations/{org.id}/events/staff/{staff_user.id}", headers=staff_headers)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert data["total"] == 1
    ...
    def test_list_staff_events_other_staff_returns_403_forbidden(
        ...
        resp = client.get(f"/organizations/{org.id}/events/staff/{other_staff.id}", headers=staff_headers)
        assert resp.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_list_staff_events_admin_can_view_any_staff(
        ...
        resp = client.get(f"/organizations/{org.id}/events/?limit=1", headers=admin_headers)
        staff_id = resp.json()["events"][0]["actor"]["id"]

        # Admin views staff events
        resp = client.get(f"/organizations/{org.id}/events/staff/{staff_id}", headers=admin_headers)
        assert resp.status_code == status.HTTP_200_OK
        assert resp.json()["total"] >= 1

    def test_list_staff_events_nonexistent_staff_returns_404(
        self, client: TestClient, admin_headers: dict, org: Organization
    ):
        """Nonexistent staff member returns 404."""
        resp = client.get(f"/organizations/{org.id}/events/staff/99999", headers=admin_headers)
        assert resp.status_code == status.HTTP_404_NOT_FOUND
...
class TestEventFiltering:
    ...
    def test_filter_by_since(self, client: TestClient, admin_headers: dict, session: Session, org: Organization):
        ...
        resp = client.get(f"/organizations/{org.id}/events/?since={future}", headers=admin_headers)
        assert resp.status_code == 200
        assert resp.json()["total"] == 0

        # Filter to past (should return the event)
        past = (datetime.now(UTC) - timedelta(hours=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
        resp = client.get(f"/organizations/{org.id}/events/?since={past}", headers=admin_headers)
        assert resp.status_code == 200
        assert resp.json()["total"] >= 1
    ...
    def test_filter_by_actor_id(
        ...
        resp = client.get(f"/organizations/{org.id}/events/?actor_id={admin_user.id}", headers=admin_headers)
        data = resp.json()
        assert data["total"] >= 1
